...
trait JumpTrait
{
    ...
    protected function success($msg = '', $data = '', $url = null, $wait = 3, array $header = [])
    {
        ...
        if ($type == 'html') {
            $result['type'] = 'html';
            $result['view'] = config('admin.dispatch_success_tmpl');
        } elseif ($type == 'json') {
            $result['type'] = 'json';
        }
        
        throw new \Exception(json_encode($result)); //只能是string 类型的
    }
    ...
    protected function error($msg = '', $data = '', $url = null, $wait = 3, array $header = [])
    {
        ...
        if ($type == 'html') {
            $result['type'] = 'html';
            $result['view'] = config('admin.dispatch_error_tmpl');
        } elseif ($type == 'json') {
            $result['type'] = 'json';
        }
    ...
    {
        return (request()->expectsJson() || request()->acceptJson() || request()->isAjax() || request()->method() == 'POST') ? 'json' : 'html';
    }
}
