...
class MenuService
{
    ...
    public function getHomeInfo()
    {
        $data = Db::name('system_menu')
            ...
            ->find();
        !empty($data) && $data['href'] = url($data['href']);
        return $data;
    }
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function getMenuTree($adminId)
    {
        /** @var AuthService $authService */
        $authServer = new AuthService($adminId);
        return $this->buildMenuChild(0, $this->getMenuData($adminId),$authServer);
    }
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    protected function getMenuData($adminId)
    {
        ...
        $where = [];
        if(!empty($adminInfo)&&$adminInfo['auth_ids'] !=7){
        	$where[] = ['isadmin','=',1];
        }
