define(["jquery", "tableSelect", "ckeditor"], function ($, tableSelect, undefined) {
    ...
    var init = {
        ...
        upload_exts: 'doc|gif|ico|icon|jpg|mp3|mp4|p12|pem|png|rar|zip',
    };
    ...
    var admin = {
        ...
        msg: {
            ...
            success: function (msg, callback) {
                ...
                var index = layer.msg(msg, {icon: 1, shade: admin.config.shade, scrollbar: false, time: 3000, shadeClose: true}, callback);
                return index;
            },
            // 失败消息
        ...
        table: {
            ...
            list: function (data, option) {
                ...
                if (option.selectList[value] === undefined || option.selectList[value] === '' || option.selectList[value] === null) {
                    ...
                    switch(value) {
                        case 0:
                            return '<span class="layui-badge layui-bg-red">'+option.selectList[value]+'</span>'
                            break;
                        case 1:
                           return '<span class="layui-badge layui-bg-blue">'+option.selectList[value]+'</span>'
                           break;
                        case 2:
                           return '<span class="layui-badge layui-bg-green">'+option.selectList[value]+'</span>'
                           break;
                        case 3:
                           return '<span class="layui-badge layui-bg-orange">'+option.selectList[value]+'</span>'
                           break;
                        case 4:
                           return '<span class="layui-badge layui-bg-cyan">'+option.selectList[value]+'</span>'
                           break;
                        case 5:
                           return '<span class="layui-badge layui-bg-gray">'+option.selectList[value]+'</span>'
                           break;
                        default:
            ...
            kuaidi: function(data,option) {
                ...
                if(data.status >= 3){
                    ...
                    var url = "//m.kuaidi100.com/index_all.html?type="+express_code+"&postid="+value;
                   
                    return '<a class="label bg-green" data-external="true" data-open="'+ url +'" data-title="'+ value +'">'+value+'</a>';
                }else{
                    return value;
                }
        ...
        api: {
            ...
            upload: function () {
                ...
                //uploadName 改成 uploadId 采用id来定位，防止name是数组的情况,remove() 删除方法不正确
                ...
                if (uploadList.length > 0) {
                    $.each(uploadList, function (i, v) {
                        ...
                        $(elem).bind("input propertychange", function (event) {
                            ...
                            $('#bing-' + uploadId).remove();
                            if (urlString.length > 0) {
                                var parant = $(this).parent('div');
                                var liHtml = '';
                                $.each(urlArray, function (i, v) {
                                    liHtml += '<li><a><img src="' + v + '" data-image  onerror="this.src=\'' + BASE_URL + 'admin/images/upload-icons/' + uploadIcon + '.png\';this.onerror=null"></a><small class="uploads-delete-tip bg-red badge" data-upload-delete="' + uploadId + '" data-upload-url="' + v + '" data-upload-sign="' + uploadSign + '">×</small></li>\n';
                                });
                                parant.after('<ul id="bing-' + uploadId + '" class="layui-input-block layuimini-upload-show">\n' + liHtml + '</ul>');
                            }
                    ...
                    $('body').on('click', '[data-upload-delete]', function () {
                        var uploadId = $(this).attr('data-upload-delete'),
